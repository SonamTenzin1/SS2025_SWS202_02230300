...
public final class TypeVariableImpl implements TypeVariable<GenericDeclaration>, Type {
    ...
    private final KTypeParameter f23012f;

    public TypeVariableImpl(KTypeParameter kTypeParameter) {
        m.f(kTypeParameter, "typeParameter");
        this.f23012f = kTypeParameter;
    }

    public boolean equals(Object other) {
        if (other instanceof TypeVariable) {
            TypeVariable typeVariable = (TypeVariable) other;
            if (m.a(getName(), typeVariable.getName()) && m.a(getGenericDeclaration(), typeVariable.getGenericDeclaration())) {
                return true;
            }
        }
        return false;
    ...
    public Type[] getBounds() {
        int u;
        Type c2;
        List<KType> upperBounds = this.f23012f.getUpperBounds();
        u = v.u(upperBounds, 10);
        ArrayList arrayList = new ArrayList(u);
        Iterator<T> it = upperBounds.iterator();
        while (it.hasNext()) {
            c2 = w.c((KType) it.next(), true);
            arrayList.add(c2);
        }
    ...
    public GenericDeclaration getGenericDeclaration() {
        throw new NotImplementedError("An operation is not implemented: " + ("getGenericDeclaration() is not yet supported for type variables created from KType: " + this.f23012f));
    }

    @Override // java.lang.reflect.TypeVariable
    public String getName() {
        return this.f23012f.getName();
    }
