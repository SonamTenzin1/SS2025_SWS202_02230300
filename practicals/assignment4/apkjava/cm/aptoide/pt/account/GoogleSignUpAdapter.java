...
public class GoogleSignUpAdapter implements SignUpAdapter<GoogleSignInResult> {
    ...
    public Single<Account> signUp(GoogleSignInResult googleSignInResult, AccountService accountService) {
        ...
        if (googleSignInResult.b() && a != null) {
            return accountService.createAccount(a.z(), a.M0(), TYPE);
        }
        return Single.h(new GoogleSignUpException(GoogleSignInStatusCodes.a(googleSignInResult.getStatus().z()), googleSignInResult.getStatus().z()));
    }
}
